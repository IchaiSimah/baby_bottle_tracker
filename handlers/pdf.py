...
TRANSLATIONS = {
    'fr': {
        ...
        'changes': 'Changements',
        'total': 'Total',
        'complete_bottles': '🍼 Liste Complète des Biberons',
        'date': 'Date',
        'time': 'Heure',
        'quantity_ml': 'Quantité (ml)',
        'complete_changes': '💩 Liste Complète des Changements',
        'information': 'Information',
        'chronological_list': '📋 Liste Chronologique Complète',
        'type': 'Type',
        'footer_line1': '📄 Rapport généré automatiquement par Baby Bottle Tracker Bot',
        'footer_line2': '📊 Données basées sur',
        'footer_line3': 'jours de suivi',
        'footer_line4': '💡 Pour plus d\'informations, contactez votre pédiatre',
        'report_days': 'Rapport',
        'days': 'jours',
        ...
        'maximum': 'Maximum',
        'minimum': 'Minimum',
        'more_info_link': 'Pour plus d\'informations sur les besoins de votre bébé, cliquez ici'
    },
    'en': {
        'title': '📊 Baby Tracking Report',
        'group': '👥 Group',
        'period': '📅 Period',
        ...
        'changes': 'Changes',
        'total': 'Total',
        'complete_bottles': '🍼 Complete Bottles List',
        'date': 'Date',
        'time': 'Time',
        'quantity_ml': 'Quantity (ml)',
        'complete_changes': '💩 Complete Changes List',
        'information': 'Information',
        'chronological_list': '📋 Complete Chronological List',
        'type': 'Type',
        'footer_line1': '📄 Report automatically generated by Baby Bottle Tracker Bot',
        'footer_line2': '📊 Data based on',
        'footer_line3': 'days of tracking',
        'footer_line4': '💡 For more information, contact your pediatrician',
        'report_days': 'Report',
        'days': 'days',
    ...
    'he': {
        'title': '📊 דוח מעקב תינוק',
        'group': '👥 קבוצה',
        'period': '📅 תקופה',
        'generated': '📄 נוצר ב',
        'general_stats': '📊 סטטיסטיקות כלליות',
        'metric': 'מדד',
        'value': 'ערך',
        'total_bottles': 'סה"כ בקבוקים',
        'total_ml': 'סה"כ מ"ל שנצרכו',
        'avg_ml_day': 'ממוצע מ"ל ליום',
        'avg_bottles_day': 'ממוצע בקבוקים ליום',
        'diaper_changes': 'החלפות חיתול',
        'daily_detail': '📅 פירוט יומי',
        'bottles': 'בקבוקים',
        'changes': 'החלפות',
        'total': 'סה"כ',
        'complete_bottles': '🍼 רשימת בקבוקים מלאה',
        'date': 'תאריך',
        'time': 'שעה',
        'quantity_ml': ')כמות )מ"ל',
        'complete_changes': '💩 רשימת החלפות מלאה',
        'information': 'מידע',
        'chronological_list': '📋 רשימה כרונולוגית מלאה',
        'type': 'סוג',
        'footer_line1': '📄 דוח נוצר אוטומטית על ידי בוט מעקב בקבוק תינוק',
        'footer_line2': '📊 נתונים מבוססים על',
        'footer_line3': 'ימי מעקב',
        'footer_line4': '💡 למידע נוסף, פנה לרופא הילדים שלך',
        'report_days': 'דוח',
        'days': 'ימים',
        'bottles_count': 'בקבוקים',
        'changes_count': 'החלפות',
        'generated_on': 'נוצר ב',
        'daily_consumption_chart': 'גרף מזומן יומי',
        'average': 'ממוצע',
        'maximum': 'מקסימום',
        'minimum': 'מינימום',
        'more_info_link': 'למידע נוסף, קראו כאן'
    }
}

def setup_hebrew_font():
    """Configure la police pour l'hébreu (NotoSansHebrew locale ou fallback système)"""
    try:
        # 1. Utiliser la police locale si elle existe
        font_path = os.path.join("assets", "fonts", "NotoSansHebrew-Regular.ttf")
        if os.path.exists(font_path):
            pdfmetrics.registerFont(TTFont('Hebrew', font_path))
            print(f"✅ Police NotoSansHebrew utilisée : {font_path}")
            return 'Hebrew'
        # 2. Fallback sur les polices système Unicode
        system_fonts = [
        ...
        for font_name in system_fonts:
            try:
                ...
                print(f"✅ Police système configurée : {font_name}")
                return 'Hebrew'
            except Exception as e:
                print(f"❌ Impossible d'utiliser {font_name}: {e}")
                continue
        # 3. Fallback ultime
        print("⚠️ Aucune police hébreu trouvée, utilisation de la police par défaut")
        return 'Helvetica'
    except Exception as e:
        print(f"❌ Erreur configuration police hébreu : {e}")
        return 'Helvetica'

async def translate_text_with_ai(text: str, target_language: str) -> str:
    """Traduit un texte avec l'IA Gemini (version optimisée)"""
    try:
...
async def translate_multiple_texts_with_ai(texts: list, target_language: str) -> list:
    """Traduit plusieurs textes en une seule requête Gemini (optimisation)"""
    try:
        # Vérifier si Gemini est disponible
        gemini_api_key = os.getenv("GEMINI_API_KEY")
        if not gemini_api_key:
        ...
        # Créer le prompt de traduction multiple
        language_names = {
            'en': 'English',
            'fr': 'French', 
            'he': 'Hebrew'
        ...
        # Créer une liste numérotée des textes à traduire
                            ...
                            if len(parts) > 0 and 'text' in parts[0]:
                                ...
                                for line in lines:
                                    ...
                                    if line and any(line.startswith(f"{i+1}.") for i in range(len(non_empty_texts))):
                                        ...
                                        # Si pas de numéro, prendre la ligne comme traduction
...
async def generate_pdf_report(update: Update, context: ContextTypes.DEFAULT_TYPE, days: int = 7, language: str = 'fr'):
    """Génère et envoie le rapport PDF dans la langue sélectionnée"""
    query = update.callback_query
    
    # Vérifier que nous avons un callback query valide
    if not query:
        print("Erreur: Pas de callback query pour la génération PDF")
        return
    ...
    if not user:
        print("Erreur: Pas d'utilisateur pour la génération PDF")
        return
    ...
    try:
        ...
        # Charger les données du groupe pour le nom
        ...
        if group_data:
            ...
            group_name = group_data[group_id].get('name', 'Mon Bébé')
            timediff = group_data[group_id].get('time_difference', 0)
        # Traduire les notes des cacas si nécessaire (optimisation : une seule requête pour toutes les notes)
        if language != 'fr':
            ...
            if poop_entries:
                ...
                # Traduire toutes les notes en une seule requête
                if notes_to_translate:
                    print(f"🌍 Traduction de {len(notes_to_translate)} notes en {language}...")
        ...
        await context.bot.send_document(
            ...
            document=pdf_buffer,
            filename=filename,
            caption=f"📄 **{TRANSLATIONS[language]['report_days']} {days} {TRANSLATIONS[language]['days']}** - {group_name}\n\n"
                   f"📊 {len(stats_data.get('entries', []))} {TRANSLATIONS[language]['bottles_count']}\n"
                   f"💩 {len(stats_data.get('poop', []))} {TRANSLATIONS[language]['changes_count']}\n"
                   f"📅 {TRANSLATIONS[language]['generated_on']} {(datetime.now(ZoneInfo('UTC')) + timedelta(hours=timediff)).strftime('%d/%m/%Y à %H:%M')}"
        )
        ...
        # Mettre à jour les informations du message dans la base de données
        if group_id:
        ...
        error_msg = "❌ Erreur lors de la génération du PDF. Veuillez réessayer."
        await query.edit_message_text(
            text=error_msg,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🏠 Accueil", callback_data="refresh")
...
async def create_weekly_pdf(stats_data, group_name, days, language='fr', timediff=0):
    """Crée le PDF avec les données de la semaine dans la langue sélectionnée"""
    buffer = BytesIO()
    ...
    # Appliquer l'inversion des caractères pour l'hébreu
    if language == 'he':
        t = {key: reverse_hebrew_text(value) for key, value in t.items()}
    
    # Configurer la police pour l'hébreu si nécessaire
    if language == 'he':
    ...
    # Déterminer l'alignement selon la langue
    if language == 'he':
    ...
    if language == 'he':
        ...
        story.append(Paragraph(f"{t['generated']} {(datetime.now(ZoneInfo('UTC')) + timedelta(hours=timediff)).strftime('%d/%m/%Y à %H:%M')}", normal_style))
    ...
    if entries:  # Seulement si il y a des données de biberons
        try:
            # Créer le graphique
            chart_buffer = create_daily_consumption_chart(stats_data, days, language)
            
            if chart_buffer.getvalue():  # Vérifier que le graphique a été créé
                ...
                link_text = t.get('more_info_link', 'Pour plus d\'informations sur les besoins de votre bébé, cliquez ici')

                
                # Créer un paragraphe avec lien cliquable
    ...
    if entries or poop:
        # Utiliser FrameBreak au lieu de PageBreak pour éviter les pages vides
        ...
        # Combiner et trier toutes les entrées par date/heure
        all_entries = []
        ...
        # Créer le tableau chronologique
        if language == 'he':
            # Pour l'hébreu, inverser l'ordre des colonnes (RTL)
            ...
            # Largeurs des colonnes inversées pour l'hébreu
        ...
        # Style du tableau avec couleurs différentes
        table_style = [
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2E86AB')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ...
        # Ajouter des couleurs différentes pour les lignes
...
async def handle_pdf_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Gère les callbacks du menu PDF"""
    query = update.callback_query
    action = query.data
...
def reverse_hebrew_text(text: str) -> str:
    """Inverse l'ordre des caractères et des mots pour le texte hébreu"""
    if not text:
        return text
    ...
    for word in words:
        # Vérifier si le mot contient des caractères hébreu
        if any('\u0590' <= char <= '\u05FF' for char in word):
            # Inverser l'ordre des caractères pour les mots hébreu
            reversed_word = word[::-1]
            processed_words.append(reversed_word)
        else:
            # Garder l'ordre normal pour les mots non-hébreu (emojis, etc.)
            processed_words.append(word)
    
    # Inverser l'ordre des mots pour l'hébreu
...
def create_daily_consumption_chart(stats_data, days: int, language: str = 'fr') -> BytesIO:
    ...
    try:
        ...
        # Configuration robuste des polices - éviter Helvetica sur Windows
        try:
            # Utiliser des polices disponibles sur tous les systèmes
            if language == 'he':
                # Pour l'hébreu, priorité à Noto Sans Hebrew
                font_path = os.path.join("assets", "fonts", "NotoSansHebrew-Regular.ttf")
                if os.path.exists(font_path):
                    rcParams['font.family'] = 'sans-serif'
                    rcParams['font.sans-serif'] = ['Noto Sans Hebrew', 'DejaVu Sans', 'Arial Unicode MS', 'Arial']
                else:
                    # Fallback sur les polices système Unicode
                    rcParams['font.family'] = 'sans-serif'
            ...
            rcParams['axes.unicode_minus'] = False  # Éviter les problèmes avec les tirets
        except Exception as e:
            print(f"⚠️ Configuration police matplotlib: {e}")
        ...
        for entry in entries:
            # Normaliser la date à UTC pour éviter les problèmes de timezone
        ...
        # Créer la liste des dates pour la période
        ...
        if language == 'he':
            ax.set_xlabel(reverse_hebrew_text(t.get('date', 'תאריך')), fontsize=12, fontweight='bold', color=secondary_color)
            ax.set_ylabel(reverse_hebrew_text(t.get('quantity_ml', 'כמות (מ"ל)')), fontsize=12, fontweight='bold', color=secondary_color)
        else:
            ax.set_xlabel(t.get('date', 'Date'), fontsize=12, fontweight='bold', color=secondary_color)
            ax.set_ylabel(t.get('quantity_ml', 'Quantité (ml)'), fontsize=12, fontweight='bold', color=secondary_color)
        ...
        if values:
            ...
            # Positionner les stats en haut à droite
